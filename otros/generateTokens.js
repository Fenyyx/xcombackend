...
export const generateTokenAndSetCookie = (userId,res) => {
    ...
    console.log(userId, token, process.env.NODE_ENV)
    res.cookie("jwt",token,{ //Mandamos una cookie al navegador
        ...
        sameSite: "None",// La cookie solo se envía si la solicitud proviene del mimsmo sitio. Para prevenir Cross site request forgery (CSRF) otro tipo de ataque, evita que cuando el user esté logeado se ejecuten acciones no buscadas
        secure: process.env.NODE_ENV !== "production", //Solo en producción las cookies se envían con HTTPS
    })
}